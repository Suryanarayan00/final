...
class Home extends Component {
  ...
  // add item to cart
  addToCart = (value) => {
    let{ cartData, count} = this.state;
    this.setState({
      count: count+1
    })

    // console.log(store.getState().shopping.cartData)
    // let cardData = store.getState().shopping.cartData;
...
const mapStateToProps = state => {
  ...
  return (
    {
      cartData: state.shopping.cartData
    }
  )
}
